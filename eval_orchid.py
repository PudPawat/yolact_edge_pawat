...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45,img_name = None):
    """
    ...
    if args.display_masks and cfg.eval_mask_branch:
        ...
        if num_dets_to_consider > 1:
            ...
            # print("masks_color_summand",masks_color_summand.shape)

        img_gpu = img_gpu * inv_alph_masks.prod(dim=0) + masks_color_summand
        print("img_gpu", img_gpu.shape)
...
def evalimage(net:Yolact, path:str, save_path:str=None, detections:Detections=None, image_id=None):
    ...
    if check_is_image_path(path):
        ...
        try:
            print("preds mask",preds[0]["mask"])
            print("preds proto",preds[0]["proto"])
            try:
                print("preds mask",preds[0]["mask"].shape)
                print("preds proto",preds[0]["proto"].shape)
            except:
                pass
        except:
            pass

        img_numpy = prep_display(preds, frame, None, None, undo_transform=False,img_name=os.path.join("orchids_mask/",img_name))
